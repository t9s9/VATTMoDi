...
from ...backbone.attention import MemoryEfficientAttention

from .helpers.vit_helpers import DropPath, trunc_normal_, build_model_with_cfg
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        # B, C, H, W = x.shape
        # if not (H == self.img_size[0] and W == self.img_size[1]) and first_RUN:
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.attn = MemoryEfficientAttention(dim,
                                             num_heads=num_heads,
                                             qkv_bias=qkv_bias,
                                             attn_drop=attn_drop,
                                             proj_drop=drop)
...
class PaSST(nn.Module):
    ...
    def __init__(self, u_patchout=0, s_patchout_t=0, s_patchout_f=0, img_size=(128, 998), patch_size=16, stride=16,
                 ...
                 act_layer=None, weight_init='', gradient_checkpointing=False):
        """
        Args:
    ...
    def forward_feature(self, x):
        ...
        if self.first_RUN: print("Embed shape", x.shape)
        if self.training and (T_dim < self.s_patchout_t or F_dim < self.s_patchout_f):
            raise ValueError("The input is too small for the given patchout size")
        ...
        if self.first_RUN: print("Orig", x.shape)
        ...
        if self.gradient_checkpointing and self.training:
            if self.first_RUN: print("Using Gradient checkpointing. Input requires_grad:", x.requires_grad)
            x = checkpoint_sequential(functions=self.blocks, input=x, segments=self.depth)
        else:
            x = self.blocks(x)
    ...
    def forward_project(self, features):
...
def get_model_passt(arch="passt_s_swa_p16_128_ap476",
                    pretrained=True,
                    n_classes=527,
                    in_channels=1,
                    fstride=10,
