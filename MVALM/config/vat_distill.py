...
def config():
    ...
    checkpoint_path = '/home/t9s9/Datasets/ckpt/VAT-Distill'
    ...
    callbacks = {
        ...
        'model_checkpoint': {
            ...
            'params': {
                ...
                # 'filename': f"{id}" + "-epoch={epoch}-step={step}-val_loss={train/total_loss:.3f}",
    ...
    model = dict(
        vision_encoder=dict(model_name='timm-vit_base_patch32_224_in21k',
    ...
    zero_shot = [
        ...
        #  'dataset': beton_root + 'evaluation/ESC50-htsat-spec.beton',
        # 'top_k': (1, 5),
    ...
    exp_name += f"{b}{caption_type}_mix{caption_mix_tag}"
...
def frozen():
    ...
    limit_val_batches = None
    model = dict(
        vision_encoder=dict(gradient_checkpointing=False,
                            freeze=True),
        text_encoder=dict(gradient_checkpointing=False,
                          freeze=True),
        audio_encoder=dict(gradient_checkpointing=False,
                           freeze=True, ))
